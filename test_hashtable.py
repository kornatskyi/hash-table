...
def test_create_hash_table():
    assert HashTable(capacity=100) is not None, "Hashtable instance is None"
...
def test_should_work_correctly_with_in_keyword(hash_table):
    assert "hola" in hash_table
    assert 98.6 in hash_table
    assert False in hash_table

def test_should_work_correctly_with_in_keyword_negative_test(hash_table):
    assert "1232" not in hash_table
    assert 98.5 not in hash_table
    assert True not in hash_table
